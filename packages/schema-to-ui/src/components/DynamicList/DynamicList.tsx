...
export const DynamicList = <T extends object>({
  ...
  const columns = React.useMemo<ColumnDef<T>[]>(() => 
    Object.entries(schema.columns).map(([key, col]) => ({
        id: key,
        accessorKey: (col as ColumnDefinition<T>).field as keyof T,
        header: (col as ColumnDefinition<T>).label,
        cell: ({ getValue, row }: CellContext<T, unknown>) => {
        ...
        enableSorting: (col as ColumnDefinition<T>).sortable,
      })),
    [schema.columns]
  );
